...
- 智能体默认直接读取用户选择的原项目，并可在完全访问模式下修改文件、执行 Shell/Bash。
...
5. 如需修改项目，直接描述目标或执行 `/optimize`；默认完全访问模式会修改原项目。
6. 重新审计修改结果，查看前后差分并导出报告。
...
系统环境变量优先于 `.env`。应用只从 `.env` 接受上述 DeepSeek 配置字段，不会将密钥写入源码、报告或交付包。其他 provider 会被拒绝。
...
- 智能体读取、搜索、Shell/Bash 和原项目写入直接作用于用户选择的路径；风险由用户承担。
- 确定性审计仍可在 `.workspaces/<session_id>/input/` 建立分析副本，不再阻塞直接读取。
...
- 默认启用完全访问模式；用户主动选择 Plan 模式时只规划、不执行。
