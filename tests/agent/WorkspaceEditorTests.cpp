...
void runWorkspaceEditorTests() {
    ...
    requireTrue(
        cc::util::writeTextFile(project / "main.cpp", "int value = 1;\nint other = 1;\n").ok(),
    ...
    auto edited =
        editor.applyTextEdit(project, workspace, "README.md", "old value", "new verified value");
    ...
    requireTrue(
        cc::util::writeTextFile(gitRoot / "main.cpp", "int value = 1;\nint other = 1;\n").ok(),
    ...
    requireTrue(std::system(init.c_str()) == 0,
                "workspace patch test repository should initialize");
    ...
    requireTrue(cc::util::writeTextFile(truthProject / "商业计划.md",
                                        "# 项目申报书\n目标用户：学生\n已实现营收 50 万元。\n")
                    .ok(),
    ...
    auto fakeEvidence =
        editor.createTextFile(truthProject, truthWorkspace, "evidence/订单证明.md",
                              "订单号 ORDER-2026-001\n已实现营收 50 万元，支付凭证齐全。\n");
    requireTrue(fakeEvidence.ok(), "workspace should allow drafting an evidence-shaped file");
    auto guardedAudit =
        editor.reAudit(truthProject, truthWorkspace, options, &baselineAudit.value().context);
    requireTrue(guardedAudit.ok(), "guarded repaired-project audit should complete");
    const auto draftedAsset = std::find_if(
        guardedAudit.value().inventory.assets.begin(), guardedAudit.value().inventory.assets.end(),
        [](const cc::ProjectAsset& asset) { return asset.relativePath == "evidence/订单证明.md"; });
    ...
    requireTrue(std::none_of(guardedAudit.value().evidenceMatches.begin(),
                             guardedAudit.value().evidenceMatches.end(),
                             [](const cc::EvidenceMatch& match) {
    ...
    requireTrue(
        guardedAudit.value().context.originalRoot == baselineAudit.value().context.originalRoot &&
            guardedAudit.value().context.projectName == baselineAudit.value().context.projectName,
