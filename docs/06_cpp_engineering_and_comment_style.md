...
写代码前必须先确定文件归属，不能把功能写进“顺手打开”的目录或文件。

```text
contest_core  = 确定性审计能力
contest_agent = 智能体运行时、工具、权限、会话
contest_llm   = 大模型请求、Brain 单步决策、迭代工具循环解析
contest-workbench = Qt/QML 展示和桥接
```

禁止：

```text
在 QML 或 CompileController 中实现扫描、规则、评分、证据匹配。
在 contest_core 中引入 LLM、联网、权限弹窗或 UI 状态。
在 contest_llm 中写文件扫描、修复执行或最终评分逻辑。
在 agent runtime 中提供万能 shell 工具。
```
...
- 一个类只负责一个明确概念；
- 一个函数只完成一个可命名步骤；
- 优先复用已有模块能力，不复制一份相似逻辑；
- 新增 helper 必须服务当前模块，不做跨域工具箱；
- 不为演示效果绕过权限、hooks、路径边界或测试；
- 不把临时产物、生成文件、工作区修订稿写进源码目录；
- 没有真实复杂度时不提前抽象，有真实重复时再抽象；
- 删除废弃分支、空 TODO、无用 include 和无用状态字段。
