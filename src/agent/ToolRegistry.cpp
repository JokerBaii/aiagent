...
[[nodiscard]] Result<void> validateProperty(const std::string& key, const JsonValue& value,
    ...
    if (type == "number" || type == "integer") {
        ...
        if (number < minimum || number > maximum) {
            ...
            message << "工具参数 " << key << " 必须在 " << minimum << " 到 " << maximum
                    << " 之间";
...
std::vector<AgentToolSpec> ToolRegistry::interactiveToolSpecs() const {
    return {
        ...
        spec("list_project_files", "枚举允许读取的项目副本文件，用于让 Brain 自动翻阅材料",
             ...
             objectSchema({{"max_files", integerProperty("最多返回文件数量，默认 80", 1,
                                                           kMaxListedFiles)}}),
             objectSchema(
        ...
        spec("inspect_project_file", "查看单个项目文件的格式、MIME、语言、大小和可用读取策略",
             ToolPermission::ReadProjectFiles,
             objectSchema({{"path", stringProperty("项目内相对路径；项目本身是单文件时可留空",
                                                     0, kMaxPathBytes)}}),
        ...
        spec("read_text_file", "读取项目副本中的文本、代码或结构化配置文件片段",
             ...
             objectSchema({{"path", stringProperty("项目内相对路径", 1, kMaxPathBytes)},
                           {"max_bytes", integerProperty("最多读取字节数，默认 12000", 1,
                                                         kMaxReadBytes)}},
        ...
        spec("read_extracted_document",
             "读取审计管线已从 PDF、DOCX、PPTX、XLSX 或结构化材料抽取的文本；"
             "保留抽取状态，不把扫描件或截断内容冒充完整正文",
             ToolPermission::ReadProjectFiles,
             objectSchema({{"path", stringProperty("项目内文档相对路径", 1, kMaxPathBytes)},
                           {"max_bytes", integerProperty("最多返回字节数，默认 24000", 1,
                                                         kMaxReadBytes)}},
                          JsonValue::Array{"path"}),
        ...
        spec("inspect_archive", "列出项目压缩包或代码包的内部条目，不解包、不执行其中内容",
             ToolPermission::ReadProjectFiles,
             objectSchema({{"path", stringProperty("项目内压缩包路径；项目本身是压缩包时可留空",
                                                     0, kMaxPathBytes)},
                           {"max_entries", integerProperty("最多返回条目数，默认 120", 1,
                                                           kMaxArchiveEntries)}}),
        ...
        spec("search_project_text", "在项目副本文本文件中搜索关键词，返回命中的路径和行号",
             ToolPermission::ReadProjectFiles,
             objectSchema({{"query", stringProperty("要搜索的文本关键词", 1, kMaxQueryBytes)},
                           {"max_files", integerProperty("最多扫描文件数量，默认 80", 1,
                                                         kMaxSearchFiles)},
                           {"max_matches", integerProperty("最多返回命中数量，默认 40", 1,
                                                           kMaxSearchMatches)},
        ...
        spec("apply_repaired_text_edit",
             "在 repaired project 中执行带旧文本锚点和命中数校验的精确修改，并更新可应用 diff",
             ToolPermission::WriteWorkspace,
             objectSchema({{"path", stringProperty("项目内相对路径", 1, kMaxPathBytes)},
                           {"expected_text", stringProperty("必须精确匹配的旧文本", 1,
                                                             kMaxEditAnchorBytes)},
                           {"replacement_text", stringProperty("替换后的文本", 0,
                                                                kMaxWorkspaceContentBytes)},
                           {"expected_occurrences",
                            integerProperty("旧文本期望命中次数，默认 1", 1,
                                            kMaxEditOccurrences)}},
        ...
        spec("create_repaired_text_file",
             ...
             ToolPermission::WriteWorkspace,
             objectSchema({{"path", stringProperty("项目内相对路径", 1, kMaxPathBytes)},
                           {"content", stringProperty("新文件内容", 1,
                                                      kMaxWorkspaceContentBytes)}},
        ...
        spec("draft_markdown_revision", "基于读取到的 Markdown 生成工作区修订稿，不覆盖原项目文件",
             ToolPermission::WriteWorkspace,
             objectSchema({{"path", stringProperty("项目内 Markdown 相对路径", 1,
                                                     kMaxPathBytes)},
                           {"instruction", stringProperty("修订目标", 0,
                                                          kMaxInstructionBytes)},
                           {"replacement_markdown",
                            stringProperty("可选，Brain 生成的完整替换稿", 0,
                                           kMaxWorkspaceContentBytes)}},
                          JsonValue::Array{"path"}),
        ...
        spec("write_workspace_file", "把 Brain 生成的文本、代码、配置或报告写入会话工作区",
             ToolPermission::WriteWorkspace,
             objectSchema({{"path", stringProperty("工作区内相对路径", 1, kMaxPathBytes)},
                           {"content", stringProperty("要写入的文本内容", 0,
                                                      kMaxWorkspaceContentBytes)}},
                          JsonValue::Array{"path", "content"}),
