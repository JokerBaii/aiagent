...
- LLM 是可选的联网辅助能力。没有密钥或没有授权时，本地规则检查仍可完整运行。
- 网络、LLM、工作区写入默认拒绝；只有当前任务的权限快照明确允许时才放行。
...
- 连续两次选择完全相同的工具和参数时，第二次不会重复执行；连续三次时停止该轮，避免无效消耗。
- 达到最大步数但强制工作流未完成时返回失败，不再用自动总结伪装成成功。
