...
本项目专注于大学生竞赛项目材料包，不做通用 Claude 克隆。它可以承担普通问答，但核心能力始终是项目缺点评审、证据核验和安全优化。

桌面端界面和流程应采用 Claude Desktop-style 会话工作区，包括侧边栏、会话主流程、composer、工具调用观察、artifact 预览、设置式权限边界、项目记忆、上下文压缩、规则包隐式匹配和 diff-first 修复流程。所有这些能力都必须服务于竞赛项目审计、补证、优化计划、二次审计和报告导出。
