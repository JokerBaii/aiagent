...
系统应支持导入项目目录、任意单文件和常见压缩包。智能体可直接读取用户选择的原路径；确定性审计对需要复制或解包的输入建立隔离工作区。

输入：项目目录、任意单文件、`.zip`、`.tar`、`.tgz`、`.tar.gz`、`.gz`、`.bz2`、`.xz`、`.zst`、`.7z` 等可安全解析的材料包。  
输出：ProjectContext、原始项目路径、工作区路径、解包状态、完整的有界文件清单、延迟内容清单和可解释警告。
...
- 能跳过 `.git` 等无关目录；
- 能标记嵌套压缩包；
- 超大、加密、链接、深路径、未知格式和容量预算外文件只降级当前条目，不能拖垮其他材料；
- 延迟文件必须保留路径、大小、格式和原因，并在 UI 与智能体清单中可见；
- 解包时防止路径穿越；
- 路径穿越、重复目标、文件/目录冲突、压缩炸弹和损坏归档必须事务回滚；
- 导入和确定性审计本身不得修改原始项目；智能体写入必须由当前权限模式单独授权。
...
- Composer 主入口使用 `/audit`、`/agent <任务>`、`/task <任务>`、`/plan [目标]`、`/optimize [目标]`、`/status`、`/compact`、`/clear`、`/help` 等命令；权限模式也可在设置中切换；
...
- 在当前项目内枚举文件、搜索文本、读取文本/Markdown；可把 Markdown 修订稿、新模板或清单写入会话工作区，在完全访问模式下也可写入原项目；
- 高风险动作经过 PermissionGate、LifecycleHookManager 和 Workbench 设置中的权限模式；
- 审计完成后必须输出缺点评审报告并提供优化入口；确定性 `RepairPlanner` 只生成优化方案、补证清单和 diff-first 草稿；`/optimize` 在 `repaired-project` 安全副本中产生真实变更，读回后执行二次审计；普通智能体任务只有在完全访问模式下才能修改原项目；
...
- 绕过 `ToolRegistry`、`PermissionGate` 或当前权限模式执行 shell；
- 在 Plan 模式下写入工作区、修改原项目、执行命令或调用网络/LLM 工具。
